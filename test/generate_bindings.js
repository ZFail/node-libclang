...
var TypeKind;
(function (TypeKind) {
    TypeKind[TypeKind["base"] = 0] = "base";
    TypeKind[TypeKind["ref"] = 1] = "ref";
    TypeKind[TypeKind["enum"] = 2] = "enum";
    TypeKind[TypeKind["struct"] = 3] = "struct";
    TypeKind[TypeKind["fn"] = 4] = "fn";
    TypeKind[TypeKind["typedef"] = 5] = "typedef";
})(TypeKind || (TypeKind = {}));
var TypescriptType;
(function (TypescriptType) {
    TypescriptType[TypescriptType["asis"] = 0] = "asis";
    TypescriptType[TypescriptType["number"] = 1] = "number";
    TypescriptType[TypescriptType["boolean"] = 2] = "boolean";
...
class FnType extends Type {
    constructor(returnType = null, args = [], exported = false) {
        super();
        this.returnType = returnType;
        this.args = args;
        this.exported = exported;
        this.kind = TypeKind.fn;
    }
}
let namedTypes = {};
let fns = [];
...
function getType(t, parentCursor = null) {
    switch (t.kind) {
...
function appendJs(content) {
    if (!args.output) {
        console.log(content);
    }
    resultJs += content + '\n';
}
function appendTs(content) {
    if (!args.output) {
        console.log(content);
    }
    resultTs += content + '\n';
}
function getJsType(t) {
...
function getTsType(t, parent = null) {
    if (t instanceof BaseType) {
    ...
    if (endBaseType && (!parent || parent.kind !== TypeKind.ref))
        return endBaseType;
...
function getTsFnReturnType(t, parent = null) {
...
function parseChild(c, spelling = undefined) {
...
function parseExportedFunctions(fileCursor, sources) {
    ...
    for (let c of cs) {
        if (!isCursorDeclaredInSources(c, sources))
            continue;
...
//# sourceMappingURL=generate_bindings.js.map